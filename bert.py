...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    scaled_S = S/math.sqrt(self.attention_head_size)
    ...
    if attention_mask is not None:
      scaled_S = scaled_S.masked_fill(attention_mask != 0, float('-inf'))
   
    # normalize the scores
    scaled_S = nn.functional.softmax(scaled_S, dim = -1)
    

    # multiply the attention scores to the value and get back V'
    V = torch.matmul(scaled_S, value)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # add-norm layer
    attention_output = self.add_norm(hidden_states,self_output, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    
    # feed forward
    intrmd_output = self.interm_dense(attention_output)
    intrmd_output = self.interm_af(intrmd_output)

    # another add-norm layer
    layer_output = self.add_norm(attention_output, intrmd_output, self.out_dense, self.out_dropout, self.out_layer_norm)
